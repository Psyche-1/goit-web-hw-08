...
def fake_send_sms(contact):
    print(f"[SMS SENDER] Надсилання SMS на номер {contact.phone_number} для {contact.fullname}...")
    time.sleep(1)
    print(f"[SMS SENDER] SMS успішно доставлено на {contact.phone_number}!")

def callback(ch, method, properties, body):
    contact_id_str = body.decode()
    print(f"\n[CONSUMER SMS] Отримано ID: {contact_id_str}")
    
    try:
        contact = Contact.objects(id=ObjectId(contact_id_str)).first()
        if contact and not contact.is_sent:
            fake_send_sms(contact)
...
print("[-] Скрипт consumer_sms.py запущено. Очікування SMS повідомлень...")
